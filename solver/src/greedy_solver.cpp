...
// ---------------------------------------------------------------------------
// Core greedy solver: per-battery regime-adjustment heuristic.
// ---------------------------------------------------------------------------
bool GreedySolver::solve(const Instance& inst, Solution& sol,
                         const loader::SolverConfig& cfg)
{
    ...
    for (const Battery& bat : inst.batteries) {
        ...
        if (!need_increase && !need_decrease) {
            utils::dbg << "[greedy] Battery " << bat.id
        ...
        if (cfg.sort_method == "priority_cost") {
            ...
            std::cerr << utils::red
                << "[greedy] Unknown sort_method '" << cfg.sort_method
                << "', using natural order.\n" << utils::reset;
        }
    ...
    utils::dbg << "[greedy] Selection summary:"
              << "  wells=" << sel_count
              << "  cost=" << sel_cost
              << "  loss=" << sel_loss_acc << "\n";
