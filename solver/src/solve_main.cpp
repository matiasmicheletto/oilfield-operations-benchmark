...
int main(int argc, char **argv) {
    ...
    // ------------------------------------------------------------------
    std::cout << "\n=== Solution ===\n";
    std::cout << "Selected wells (" << sol.selected_ids.size() << "):";
    for (int id : sol.selected_ids) std::cout << " " << id;
    std::cout << "\n";

    std::cout << "Route (depot=0):";
    for (int node : sol.route) std::cout << " " << node;
    std::cout << "\n";

    std::cout << "Total travel time: " << sol.total_distance << "\n";

    if (!cfg.output_file.empty()) {
        std::ofstream out(cfg.output_file);
        if (out) {
            out << "selected_wells:";
            for (int id : sol.selected_ids) out << " " << id;
            out << "\nroute:";
            for (int node : sol.route) out << " " << node;
